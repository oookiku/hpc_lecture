...
int main (int argc, char** argv) {
  ...
  hsize_t Nlocal4[2] = {NX/dim[0]/2, NY/dim[1]/2};
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= Nlocal4[i];
  hsize_t count[2] = {2,2};
  hsize_t stride[2] = {2*Nlocal4[0], 2*Nlocal4[1]};
  ...
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal4);
  H5Pclose(plist);
